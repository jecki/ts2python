...
project's home-directory from where the ts2pyhon-command will
automatically apply them without the need of specifying any
particular command line options every time it is called.

From the command line, generating Python-pendants for Typescript-interfaces
...
Other serialiazations for the AST are available: SXML (S-expression),
ndst, json or simply an indented tree.
