...
if sys.version_info < (3,14):
    ...
    class _TypedDictMeta(type):
        def __new__(cls, name, bases, ns, total=True):
            """Create a new typed dict object.
    ...
    if sys.version_info >= (3, 11) and not _is_pypy:
        ...
        def TypedDict(typename, fields=None, *, total=True, **kwargs):
            ...
            relying on the `total`-parameter, allows treating individual fields
            as not required by declaring their type as optional. (The usage of the
            type-modifier "Optional" (for Python versions < 3.11, starting with Python
            version 3.11 the new added "NotRequired" type-marker is the preferred
            alternative.) for this purpose is in fact one of the alternatives
            rejected by PEP 655.

            TypedDict is a simple typed namespace. At runtime it is equivalent to a
            ...
            if module is not None:
                # Setting the correct module is necessary to make typed dict classes pickleable.
